# ==========================================
# MGV PAINTERS SOCKET-BASED WEB SERVER (V9-TEMP)
# ==========================================

$port = 8892
$address = [System.Net.IPAddress]::Loopback
$server = New-Object System.Net.Sockets.TcpListener($address, $port)
$logPath = "server_debug_8892.log"
...
try {
    ...
    $server.Start()
    Log-Msg "Server started and listening on 127.0.0.1:$port"
    ...
    while ($true) {
        ...
        try {
            ...
            # Check for available data with a timeout (max 60 seconds / 60000ms)
            $waitCount = 0
            while (-not $stream.DataAvailable -and $waitCount -lt 600) {
